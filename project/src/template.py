...
import solver
from core import BusMeasurement, Switch, create_switchable_line_between
...
def create_simple_network() -> pandapowerNet:
    ...
    trafo = pp.create_transformer(
        net, hv_bus=bus0, lv_bus=bus1, std_type="0.4 MVA 20/0.4 kV", name="Trafo"
    )
    line = pp.create_line(
        net,
        from_bus=bus1,
    ...
    pp.create_switch(
        net, bus=bus2, element=line2, et="l", closed=False, name="Switch Line 1"
    )
    pp.create_switch(
        net, bus=bus3, element=line3, et="l", closed=False, name="Switch Line 2"
    )
    pp.create_switch(
        net, bus=bus4, element=line5, et="l", closed=False, name="Switch Line 3"
    )
    return net
