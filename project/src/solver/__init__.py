...
def solve(
    ...
    communication_topology = map_busmeasurements_and_switches_to_nodes(communication_topology,
                                                                       net,
                                                                       bus_measurements,
                                                                       switches)
...
def create_communication_topology(
    open_network: nx.Graph,
    closed_network: nx.Graph
) -> nx.Graph:
...
def create_agents(communication_topology: nx.Graph) -> list[Agent]:
    for i, node in enumerate(communication_topology.nodes):
        communication_topology.nodes[node]['agent_address'] = (
            mango.AgentAddress(ADDRESS, f'agent{i}'))

    for node in communication_topology.nodes:
        neighboring_nodes = communication_topology.neighbors(node)
        neighbors = [communication_topology.nodes[neighbor]['agent_address']
